...
import {Vault, IRebaseToken} from "../src/Vault.sol";
...
contract TokenAndPoolDeployer is Script {
    function run() public returns (RebaseToken token, RebaseTokenPool pool) {
        ...
        Register.NetworkDetails memory networkDetails = localSimulatorFork.getNetworkDetails(block.chainid);
...
contract VaultDeployer is Script {
    function run(address _rebaseToken) public returns (Vault vault) {
        ...
        IRebaseToken(_rebaseToken).grantMintAndBurnRoles(address(vault));
